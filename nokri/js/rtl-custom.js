...
// Candidate saving job alerts model
    ...
    function sbDropzone_comp_image_location() {
        ...
        var fileList = new Array;
        var i = 0;
        $("#company-location-dropzone").dropzone({
            addRemoveLinks: true,
            paramName: "my_file_upload",
            maxFiles: $('#sb_upload_limit').val(), //change limit as per your requirements
            acceptedFiles: '.jpeg,.jpg,.png',
            dictMaxFilesExceeded: $('#adforest_max_upload_reach').val(),
            /*acceptedFiles: acceptedFileTypes,*/
            url: nokri_ajax_url + "?action=nokri_upload_comp_image_location&is_update=" + $('#is_update').val(),
            parallelUploads: 1,
            dictDefaultMessage: $('#dictDefaultMessage').val(),
            dictFallbackMessage: $('#dictFallbackMessage').val(),
            dictFallbackText: $('#dictFallbackText').val(),
            ...
            init: function () {
                var thisDropzone = this;
                $.post(nokri_ajax_url, {
                    action: 'get_uploaded_company_images_location',
                }).done(function (data) {
                    if (data != 0) {
                        $.each(data, function (key, value) {
                            var mockFile = {
                ...
                this.on("removedfile", function (file) {
                    ...
                    if (img_id != "") {
                        ...
                        if (i == 0)
                            $('.dz-message').show();
                        $.post(nokri_ajax_url, {
                            action: 'delete_comp_image_location',
                            img: img_id,
                            is_update: $('#is_update').val(),
                        }).done(function (response) {
                            if ($.trim(response) == "1") {
            ...
            var ok = $('.parsley-error').length === 0;
                ...
                .on('form:submit', function () {
                        ...
                        action: 'sb_post_user_ratting', sb_data: $("form#user_ratting_form").serialize(), security: $('#rating_nonce').val()}).done(function (response)
                    {
                        ...
                        if ($.trim(res_arr[0]) != "0")
                        {
                            toastr.success(res_arr[1], '', {timeOut: 2500, "closeButton": true, "positionClass": "toast-top-left"});
                            window.location.reload(true);
                        } else
                        {
                            toastr.error(res_arr[1], '', {timeOut: 2500, "closeButton": true, "positionClass": "toast-top-left"});

                        }
                    }).fail(function () {
                        $('#sb_loading').hide();
                        toastr.error($('#nonce_error').val(), '', {timeOut: 4000, "closeButton": true, "positionClass": "toast-top-left"});
                    });
    ...
    $(document).on('click', '#sb_verification_ph_code', function ()
    {
        ...
        $.post(nokri_ajax_url, {action: 'nokri_verification_system', code_entered: code}).done(function (response)
        {
            ...
            if ($.trim(res_arr[0]) == "2")
            {
                ...
                toastr.success(res_arr[1], '', {timeOut: 4000, "closeButton": true, "positionClass": "toast-top-left"});
            ...
            } else
            {
                $('#sb_verification_processing').hide();
                $('#sb_verification_ph_code').show();
                toastr.error(res_arr[1], '', {timeOut: 4000, "closeButton": true, "positionClass": "toast-top-left"});
            }
        });
                    });
            ...
            browsed_img.change(function () {
                ...
                if (files && files.length) {
                    ...
                    if (/^image\/\w+$/.test(file.type)) {
                        if (cur_url) {
                            URL.revokeObjectURL(cur_url); // Revoke the old one
                        }
