...
class Nokri_Pdf2 extends TCPDF {
...
if ($cand_education && $cand_education[0]['degree_name'] != '') {
    ...
    foreach ($cand_education as $edu) {
        ...
        $degre_insti = (isset($edu['degree_institute'])) ?   esc_html($edu['degree_institute']) : '';
        ...
        $cand_education_html .= '<div class="resume-timeline-box">
                <span class="degree-duration">
                    ' . esc_html($degre_name) ."-" .$degre_insti. "(" . esc_html($degre_strt) . "   " . "-" . "   " . esc_html($degre_end) . ")" . '                  
                </span>
           ' . ($degre_details) . '
            </div>';
    }
...
if ($cand_profession && $cand_profession[0]['project_organization'] != '') {
    ...
    foreach ($cand_profession as $profession) {
        ...
        $project_org = (isset($profession['project_organization'])) ?  $profession['project_organization']  : '';
        ...
        $cand_profession_html .= '<div class="resume-timeline-box">
                <span class="degree-duration">
                   ' . esc_html($project_role) ."-" .$project_org."(" . esc_html($project_strt) . "   " . "-" . "   " . esc_html($project_end) . ")" . '                     
                </span>
           ' .  ($project_detail) . '
            </div>';
    }
...
$pdf = new Nokri_Pdf2(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
...
$pdf->SetLineStyle( array( 'width' => 15, 'color' => array(221,237,237)));
...
$pdf->writeHTMLCell(65, $yx, 8, 8, $cand_dp . $about_sec . $skills_bar, 0, 0, 1, true, 'J', true);
...
$pdf->writeHTMLCell($pw - 80, '', 75, '', $html, 0, 1, 0, true, 'J', true);
