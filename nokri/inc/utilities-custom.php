    ...
    function nokri_job_post_taxonomies($taxonomy_name = '', $value = '') {
        ...
        if (count((array) $taxonomies) > 0) {
            foreach ($taxonomies as $taxonomy) {
                ...
                if( $is_value_array ){
                    $selected = ( in_array( $taxonomy->term_id, $value ) ? 'selected="selected"' : '' );
                }else{
...
if (!function_exists('nokri_job_search_taxonomies_checkboxes')) {
    ...
    nokri_job_search_taxonomies_checkboxes($taxonomy_name = '', $instance) {
        ...
        global $nokri;
        $taxonomies = get_terms($taxonomy_name, array('hide_empty' => false, 'orderby' => 'id', 'order' => 'ASC', 'parent' => 0, 'include' => $taxonomies_ids ));
        ...
        if (count((array) $taxonomies) > 0) {
            ...
            foreach ($taxonomies as $taxonomy) {
                ...
                if ($max_record < $count && $is_show && $showed) {
                    $showed = false;
                    $more_html = '<li class="show-more"><small><a href="javascript:void(0);" class="show_next" data-tax-id="' . esc_attr($taxonomy_name) . '">' . __('', 'nokri') . '</a></small><li>';
                }
                if ($multi_searach) {
                    $name = $taxonomy_name . "[]";
                    $option .= '<li class="' . esc_attr($toggle) . '"><input class="' . $search_class . ' change_select" value = "' . esc_attr($taxonomy->term_id) . '" name = "' . $name . '" type="checkbox" ' . esc_attr($selected) . ' ><label>' . esc_html($taxonomy->name) . '</label></li>';
                } else {

                    $option .= '<option class="' . $search_class . ' change_select" value = "' . esc_attr($taxonomy->term_id) . '" name = "' . $taxonomy_name . '" ' . esc_attr($selected) . ' ><label>' . esc_html($taxonomy->name) . '</label>';
                }
                $count++;
            }
            $option .= $more_html;
        ...
        if ($multi_searach) {
            ...
            $form = '<form  method="get"  action="' . get_the_permalink($nokri['sb_search_page']) . '" >
                                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="Select Option" name="' . esc_attr($taxonomy_name) . '">
        ...
        return '<div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                                   <div class="panel-heading" >
                                      <label>' . $title . '</label>
                                    </div>
                                    <div id="' . esc_attr($taxonomy_name) . '">
...
if (!function_exists('nokri_apply_with_external_source')) {
    ...
    function nokri_apply_with_external_source($job_id = '') {
        ...
        if ($job_apply_with == 'exter') {
            $apply_button = '<a style="display: none;" href="' . $href . '" class="whitebutton n-btn-flat btn-mid btn-clear  ' . $exter_app . ' " data-job-id="' . esc_attr($job_id) . '"  data-job-exter="' . ( $job_apply_url ) . '">' . esc_html($apply_now_text) . '</a>';
        } else if ($job_apply_with == 'mail') {
            $apply_button = '<a href="' . $href . '" class="whitebutton n-btn-flat btn-mid btn-clear ' . $email_app . '" data-job-id="' . esc_attr($job_id) . '" data-job-exter="' . ( $job_apply_mail ) . '">' . $apply_now_text . '</a>';
        } else if ($job_apply_with == 'whatsapp') {
            $apply_button = '<a href="' . $href_whatsapp . '" class="whitebutton n-btn-flat btn-mid btn-clear ' . $whatspp_app . '" data-job-id="' . esc_attr($job_id) . '" data-job-exter="' . ( $job_apply_whatsapp ) . '">' . $apply_now_text . '</a>';
        } else {
            $apply_button = '<a href="' . $href . '" class="whitebutton n-btn-flat ' . $simple_app . '" data-toggle="modal" data-target="' . $modal_target . '"  data-job-id=' . esc_attr($job_id) . '>' . esc_html($apply_now_text) . ' </a>';
        }
        return $apply_button;
    }
    ...
    function nokri_job_post_single_taxonomies($taxonomy_name = '', $value = '') {
        ...
        if( is_array( $value ) ){
            ...
            foreach( $value as $val ){
                $taxonomies_single = get_term_by('id', $val, $taxonomy_name);
                
                if ($taxonomies_single) {
                    $text[] = $taxonomies_single->name;
                }
            }
            return implode( ', ', $text );
        }else{
...
if (!function_exists('nokri_get_resume_publically')) {
    ...
    function nokri_get_resume_publically($user_id = '', $type = '') {
        ...
        if (!empty($ids_array)) {
            ...
            $download_btn = '<a class="solidwhitebutton n-btn-flat n-btn-custom btn-block" target="_blank" href="' . $final_url . '&download_file=1">' . esc_html__('View Resume', 'nokri') . '</a>';
        }
        if ($type == 'id') {
            return $attach_id;
        } else {
    ...
    function nokri_premium_job_class_badges($job_id = '') {
        ...
        if (count((array) $single_job_badges) > 0) {
            ...
            foreach ($single_job_badges as $job_badge => $val) {
                ...
                $style_li = $style_anch = '';
                if ($color != "") {
                    $style_li = 'style=" background-color: transparent !important;"';
                    $style_anch = 'style="color: #09283c !important;"';
                }
                ...
                $job_badge_text .= '<li ' . $style_li . '><a href="' . esc_url(nokri_page_lang_url_callback($search_url)) . '" class="job-class-tags-anchor" ' . $style_anch . '><i class="fa fa-check"></i>' . esc_html(ucfirst($terms->name)) . '</a></li>';
            }
...
/* * *************************************** */
/* Getting Job Selected Seasons For Job */
/* * *************************************** */
if (!function_exists('nokri_job_selected_seasons')) {

    function nokri_job_selected_seasons($taxonomy_name = '', $meta_key = '', $job_seasons = '') {
        $taxonomies = get_terms($taxonomy_name, array('hide_empty' => false, 'orderby' => 'id', 'order' => 'ASC', 'parent' => 0));
        $option = '';
        if (empty($job_seasons)) {
            $job_seasons = array();
        }
        ...
        if (count((array) $menus) > 0) {
            foreach ($menus as $optn => $valu) {
                if (($optn == 'dashboard' || $optn == 'Dashboard') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . get_the_permalink() . '"><span class="la la-tachometer"></span>' . ($valu) . '</a></li>';
                    $label = esc_html__('Applied Candidates', 'nokri');
                    $sorter .= '<li><a class="profile-menu-link" href="' . nokri_set_url_param(get_the_permalink(), 'tab-data', 'candidates') . '"><span class="la la-user-secret"></span> '.$label.'</a></li>';
                }
                ...
                if (($optn == 'update_profile' || $optn == 'Udpdate Profile' ) && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($dashboard_update_url)) . '"><span class="la la-edit"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'view_my_profile' || $optn == 'View My Profile' ) && $valu != "") {
                    $label = isset($valu) ? $valu : esc_html__('View My Profile', 'nokri');
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(get_author_posts_url($user_id)) . '"><span class="la la-user"></span>' . ($label) . '</a></li>';
                }
                ...
                if (($optn == 'matched_resumes' || $optn == 'Matched Resumes') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($matched_resume_url)) . '"><span class="la la-external-link"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'saved_resumes' || $optn == 'Saved Resumes' ) && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($saved_resume_url)) . '"><span class="la la-file-pdf-o"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'followers' || $optn == 'Followers') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($followers_url)) . '"><span class="fa fa-users"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'my_package' || $optn == 'My Package') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($packages_url)) . '"><span class="la la-list-alt"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'my_rating' || $optn == 'My Ratings') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($packages_url)) . '"><span class="la la-star"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'my_orders' || $optn == 'My Orders') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url('/account/orders/') . '"><span class="la la-check-circle"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'cart' || $optn == 'Cart') && $valu != "") {
                    if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
                        global $woocommerce;
                        $label = isset($valu) ? $valu : esc_html__('Cart', 'nokri');
                        $sorter .= '<li><a class="profile-menu-link" href="' . wc_get_cart_url() . '"><span class="la la-shopping-cart"></span>' . ($label) . '</a></li>';
                    }
                }
                if (($optn == 'logout' || $optn == 'Logout') && $valu != "") {
                    $label = isset($valu) ? $valu : esc_html__('Logout', 'nokri');
                    $sorter .= '<li><a class="profile-menu-link" href="' . wp_logout_url(home_url()) . '"><span class="la la-sign-out"></span>' . ($label) . '</a></li>';
                }
            }
        }
        return $sorter;
    ...
    function nokri_candidate_menu_sorter($user_id = '') {
        ...
        if (count((array) $menus) > 0) {
            foreach ($menus as $optn => $valu) {
                if (($optn == 'dashboard' || $optn == 'Dashboard') && $valu != "") {
                    $label = isset($valu) ? $valu : esc_html__('Dashboard', 'nokri');
                    $sorter .= '<li><a class="profile-menu-link" href="' . get_the_permalink() . '"><span class="la la-tachometer"></span>' . ($label) . '</a></li>';
                }
                ...
                if (($optn == 'my_resumes' || $optn == 'My Resumes') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($resumes_list)) . '"><span class="la la-file-pdf-o"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'jobs_applied' || $optn == 'Jobs Applied') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($jobs_applied)) . '"><span class="la la-bars"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'saved_jobs' || $optn == 'Saved Jobs') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($saved_jobs)) . '"><span class="la la-bookmark"></span>' . ($label) . '</a></li>';
                }

                if (($optn == 'my_rating' || $optn == 'My rating') && $valu != "") {
                    $label = isset($valu) ? $valu : esc_html__('My ratings', 'nokri');
                    $my_rating = nokri_set_url_param(get_the_permalink(), 'candidate-page', 'my_ratings');
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($my_rating)) . '"><span class="la la-star"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'followed_companies' || $optn == 'Followed Companies') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($followed_companies)) . '"><span class="la la-external-link"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'job_alerts' || $optn == 'Job Alerts') && $valu != "" && $job_alerts) {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($job_alerts)) . '"><span class="la la-bell"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'my_package' || $optn == 'My Package' ) && $valu != "" && $is_apply_pkg_base) {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($my_packages)) . '"><span class="la la-list-alt"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'my_orders' || $optn == 'My Orders') && $valu != "" && $is_apply_pkg_base) {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url(nokri_page_lang_url_callback($my_orders)) . '"><span class="la la-check-circle"></span>' . ($label) . '</a></li>';
                }
                if (($optn == 'cart' || $optn == 'Cart') && $valu != "") {
                    if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
                        global $woocommerce;
                ...
                if (($optn == 'settings' || $optn == 'setting') && $valu != "") {
                    ...
                    $sorter .= '<li><a class="profile-menu-link" href="' . esc_url($followed_companies) . '"><span class="la la-wrench"></span>' . ($label) . '</a></li>';
                }
            }
        }
        return $sorter;
    ...
    function nokri_job_categories_with_chlid_no_href($pid, $taxonomy = '') {
        ...
        if( $taxonomy == 'ad_location' ){
            ...
            foreach ($childs as $child) {
                ...
                $cats_html .= esc_html( $child->name ) . "" . ', '.$parent[$child->parent]->name.' <span class="location-separator">•</span> ';
            ...
            foreach ($custom_feilds as $value) {
                ...
                if (!$show_profile) {
                    ...
                    if ($field_type == 'Text Area') {
                        ...
                        $custom_feilds_html .= '<div class="col-md-12 col-xs-12 col-sm-12 text-input-box">
													<div class="form-group">
														<label class="">' . esc_html__($field_label) . '</label>
														<textarea  name="_custom_[' . $field_slug . ']" class="form-control jquery-textarea"   cols="30" rows="10" ' . ($requires) . '>' . esc_html($meta_value) . '</textarea>
													</div>
												</div>';
                    }
                    /* Select Box */
                    if ($field_type == 'Select Box') {
                        ...
                        $field_slug = preg_replace('/\s+/', '', $field_label);
                        $meta_value = get_user_meta($user_id, $field_slug, true);
                        $options = '<option value = "">' . esc_html__('Select option','nokri');
                        ...
                        $custom_feilds_html .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
														<div class="form-group">
															<label>' . esc_html($field_label) . '</label>
																<select data-allow-clear="true" data-placeholder="Select Option" class="js-example-basic-single" name="_custom_[' . $field_slug . ']" ' . $requires . '>
																	' . ($options) . ' 
																</select>
														</div>
													</div>';
                    ...
                    if ($field_pub == 'Yes') {
                        ...
                        $meta_value = get_user_meta($user_id, $field_slug, true);
                        if ($meta_value != '')
                            $custom_feilds_html .= '<li><small>' . $field_label . '</small><p>' . $meta_value . '</li></p>';
                        else
                            $custom_feilds_html .= '';
                    }
                }
...
if (!function_exists('nokri_get_questions_answers')) {
    ...
    function nokri_get_questions_answers($job_id = '', $candidate_id = '') {
        ...
        $cand_answers = get_user_meta($candidate_id, '_job_answers_' . $candidate_id .'_'.$job_id, true);
...
function nokri_return_horizontal_search_bar() {
    ...
    <li class="btn-filter-search">
        <div class="form-group">
            <div class="form-btn">
                <input type="submit" class="btn n-btn-flat"  value="Search">
...
if (!function_exists('nokri_fetch_business_hours')) {
    ...
    function nokri_fetch_business_hours($user_id) {
