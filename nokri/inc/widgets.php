    ...
    public function widget($args, $instance) {
        ...
        if (taxonomy_exists('job_category')) {
            ...
            if (count((array) $ad_cats) > 0) {
                $cats_html = '';
                $cats_html .= '<option value="asa">' . esc_html__('Select Option', 'nokri') . '</option>';
                    ...
                    <div id="collapse-job_category"  class="panel-collapse collapse <?php echo esc_attr($in); ?>" role="tabpanel" >
                        <div class="panel-body">
                            ...
                            <form method="get" id="search_form" action="<?php echo get_the_permalink($nokri['sb_search_page']); ?>">
                                ...
                                <input type="button" class="bluebutton n-btn-flat btn-mid" id="category_search" value="<?php echo esc_html__('Search', 'nokri'); ?>">
                                <input type="hidden" name="cat-id" id="cat_id" value="" />
                                <?php if(!$multi_searach){ echo nokri_search_params('cat-id'); } ?>
                                <?php echo nokri_form_lang_field_callback(true) ?>asdf
                            </form>
                        </div>
                    </div>
                </div>
...
class nokri_search_location extends WP_Widget {
    ...
    public function widget($args, $instance) {
        ...
        $postresults = get_posts($args);
        $taxonomy_name = 'ad_location';
        
        $taxonomies_ids = array();
        foreach( $postresults as $job ){
            $term_list = wp_get_post_terms( $job->ID, $taxonomy_name, array( 'fields' => 'ids' ) );
            if( ! empty( $term_list ) ){
                $taxonomies_ids = array_unique (array_merge ($taxonomies_ids, $term_list));
            }
        ...
        $job_states = nokri_get_cats('ad_location', $country_id);

        $state_html = '';
        $states = array();
        if (count((array) $job_states) > 0) {
            $state_html .= '<option value="">' . esc_html__('Select Option', 'nokri') . '</option>';
            foreach ($job_states as $job_state) {
                if( in_array( $job_state->term_id, $taxonomies_ids) ) {
                    if ( ! isset($_GET['job-location-state']) || $_GET['job-location-state'] == '' ) {
                        $states[] = $job_state->term_id;
                    }
                    
                    $selected = '';
                    if (isset($_GET['job-location-state']) && $_GET['job-location-state'] == $job_state->term_id) {
                        $selected = 'selected = "selected"';
                        $states[] = $job_state->term_id;
                    }
                    $state_html .= '<option value="' . esc_attr($job_state->term_id) . '" ' . $selected . '>' . esc_html($job_state->name) . '</option>';
                }
        ...
        <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
            ...
            <div id="location_search">
                    ...
                    <form method="get" id="search_form_state" action="<?php echo "" . ($form_for); ?>">
                        <div class="cp-loader"></div>
                        <div class="form-group">
                        ...
                        <?php if(!$multi_searach) {echo nokri_search_params('job-location-state'); }?>
                        <?php echo nokri_form_lang_field_callback(true);  ?>
                    </form>
        ...
        if (count((array) $states) > 0) {
            foreach( $states as $state ) {
                ...
                $job_cities = nokri_get_cats('ad_location', $state);
                $city_html .= '<option value="">' . esc_html__('Select Option', 'nokri') . '</option>';
                foreach ( $job_cities as $job_city) {
                    if( in_array( $job_city->term_id, $taxonomies_ids) ) {
                        $selected = '';
                        if (isset($_GET['job-location-city']) && $_GET['job-location-city'] == $job_city->term_id) {
                            $selected = 'selected = "selected"';
                        }
                        $city_html .= '<option value="' . esc_attr($job_city->term_id) . '" ' . $selected . '>' . esc_html($job_city->name) . '</option>';
                    }
                }
            }
        ...
        <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
            ...
            <div id="location_search_city">
                    ...
                    <form method="get" id="search_form_city" action="<?php echo "" . ($form_for); ?>">
                        <div class="cp-loader"></div>
                        <div class="form-group">
                        ...
                        <?php if(!$multi_searach) {echo nokri_search_params('job-location-city'); }?>
                        <?php echo nokri_form_lang_field_callback(true);  ?>
                    </form>
    ...
    public function widget($args, $instance) {
        ...
        if (count((array) $search_types) > 0) {
            ...
            foreach ($search_types as $search_type) {
                ...
                if ($showed) {
                    $showed = false;
                    $type_html .= '<li class="show-more hide_now_' . esc_attr($cur_cls) . '"><small><a href="javascript:void(0);" class="show_records" data-attr-id="hide_type" data-attr-hide="' . esc_attr($cur_cls) . '">' . __('', 'nokri') . '</a></small></li>';
                }
    ...
    public function widget($args, $instance) {
        ...
        if (count((array) $search_types) > 0) {
            ...
            foreach ($search_types as $search_type) {
                ...
                if ($showed) {
                    $showed = false;
                    $type_html .= '<li class="show-more hide_now_' . esc_attr($cur_cls) . '"><small><a href="javascript:void(0);" class="show_records" data-attr-id="hide_level" data-attr-hide="' . esc_attr($cur_cls) . '">' . __('', 'nokri') . '</a></small></li>';
                }
    ...
    public function widget($args, $instance) {
        ...
        if (count((array) $search_types) > 0) {
            ...
            foreach ($search_types as $search_type) {
                ...
                if ($showed) {
                    $showed = false;
                    $type_html .= '<li class="show-more show-more hide_now_' . esc_attr($cur_cls) . '"><small><a href="javascript:void(0);" class="show_records" data-attr-id="hide_experience" data-attr-hide="' . esc_attr($cur_cls) . '">' . __('', 'nokri') . '</a></small></li>';
                }
    ...
    public function widget($args, $instance) {
        ...
        if (count((array) $search_types) > 0) {
            ...
            foreach ($search_types as $search_type) {
                ...
                if ($showed) {
                    $showed = false;
                    $type_html .= '<li class="show-more show-more hide_now_' . esc_attr($cur_cls) . '"><small><a href="javascript:void(0);" class="show_records" data-attr-id="hide_qualification" data-attr-hide="' . esc_attr($cur_cls) . '">' . __('', 'nokri') . '</a></small></li>';
                }
    ...
    public function widget($args, $instance) {
        ...
        if (count((array) $search_types) > 0) {
            ...
            foreach ($search_types as $search_type) {
                ...
                if ($showed) {
                    $showed = false;
                    $salary_html .= '<li class="show-more hide_now_' . esc_attr($cur_cls) . '"><small><a href="javascript:void(0);" class="show_records" data-attr-id="hide_salary" data-attr-hide="' . esc_attr($cur_cls) . '">' . __('', 'nokri') . '</a></small></li>';
                }
    ...
    public function widget($args, $instance) {
        ...
        if (count((array) $search_types) > 0) {
            ...
            foreach ($search_types as $search_type) {
                ...
                if ($showed) {
                    $showed = false;
                    $salary_html .= '<li class="show-more hide_now_' . esc_attr($cur_cls) . '"><small><a href="javascript:void(0);" class="show_records" data-attr-id="hide_salary_type" data-attr-hide="' . esc_attr($cur_cls) . '">' . __('', 'nokri') . '</a></small></li>';
                }
    ...
    public function widget($args, $instance) {
        ...
        if (count((array) $search_types) > 0) {
            ...
            foreach ($search_types as $search_type) {
                ...
                if ($showed) {
                    $showed = false;
                    $salary_html .= '<li class="show-more hide_now_' . esc_attr($cur_cls) . '"><small><a href="javascript:void(0);" class="show_records" data-attr-id="hide_salary_currency" data-attr-hide="' . esc_attr($cur_cls) . '">' . __('', 'nokri') . '</a></small></li>';
                }
...
class nokri_search_employer_skills extends WP_Widget {
     ...
     * @param array $instance Saved values from database.
     */
    public function widget($argss, $instance) {
        global $args;
        ...
        foreach( $users as $user ){
            $metas = get_user_meta( $user->ID, '_emp_skills', true );
            $userterms = array_unique (array_merge ($userterms, $metas));
        }
        
        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Industries */
        $search_types = nokri_get_cats2('emp_specialization', $userterms , 0);
        ...
        if (count((array) $search_types) > 0) {
            ...
            foreach ($search_types as $search_type) {
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_skills"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                <div class="panel-heading">
                    <label>
                        <?php
                        if (!empty($instance['title'])) {
                            echo nokri_returnEcho($argss['before_title'] . apply_filters('widget_title', $instance['title']) . $argss['after_title']);
                        }
                        ?>
                    </label>
                </div>
                <div id="emp_job_skills">
                    <form method="get" id="emp_job_skills"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_skills">
...
/* ========================= */
/*  Employer application widget  */
/* ========================= */

class nokri_search_employer_application extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_application', // Base ID
                esc_html__('Employer application', 'nokri'), // Name
                array('description' => esc_html__('Show employer by application', 'nokri'),) // Args
        );
    }
    ...
    public function widget($args, $instance) {
        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer application */
        $search_types = nokri_get_cats('emp_specialization_application', 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_application']) && $_GET['emp_application'] == $search_type->term_id) {
                    $selected = 'checked = "checked"';
                }
                ...
                $skill_html .= '<li class="' . esc_attr($cls) . '"><input class="input-icheck input-icheck-search cand_skills_form" ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" type="radio"  name="emp_application"> <label>' . esc_html($search_type->name) . '</label></li>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            if (isset($_GET['emp_application']) && $_GET['emp_application'] != "") {
                $in = 'in';
            }
            ?>
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                <div class="panel-heading active" role="tab" >
                    <a role="button" data-toggle="collapse" href="#emp_job_application">
                        <?php
                        if (!empty($instance['title'])) {
                            echo nokri_returnEcho($args['before_title'] . apply_filters('widget_title', $instance['title']) . $args['after_title']);
                        }
                ...
                <div id="emp_job_application"  class="panel-collapse collapse <?php echo esc_attr($in); ?>" role="tabpanel">
                    <form method="get" id="emp_job_application"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <div class="panel-body">
                            <ul class="list">
                                <?php echo "" . ($skill_html); ?>
                            </ul>
                        </div>
                        <?php echo nokri_search_params('emp_application'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_application() {
    register_widget('nokri_search_employer_application');
}

add_action('widgets_init', 'nokri_search_employer_application');


/* ========================= */
/*  Employer housing widget  */
/* ========================= */

class nokri_search_employer_housing extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_housing', // Base ID
                esc_html__('Employer Housing', 'nokri'), // Name
                array('description' => esc_html__('Show employer by housing', 'nokri'),) // Args
        );
    }
    ...
    public function widget($argss, $instance) {
        ...
        $userterms = array();
        foreach( $users as $user ){
            $metas = get_user_meta( $user->ID, '_emp_housing', true );
            $userterms = array_unique (array_merge ($userterms, $metas));
        }

        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Housing */
        $search_types = nokri_get_cats2('emp_specialization_housing', $userterms , 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_housing']) && $_GET['emp_housing'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '"  name="emp_housing"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_housing">
                    <form method="get" id="emp_job_housing"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_housing">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>

                        <?php echo nokri_search_params('emp_housing'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_housing() {
    register_widget('nokri_search_employer_housing');
}

add_action('widgets_init', 'nokri_search_employer_housing');


/* ========================= */
/*  Employer Pets Allowed widget  */
/* ========================= */

class nokri_search_employer_pets extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_pets', // Base ID
                esc_html__('Employer Pets Allowed', 'nokri'), // Name
                array('description' => esc_html__('Show employer by pets', 'nokri'),) // Args
        );
    }
    ...
    public function widget($argss, $instance) {
        ...
        $userterms = array();
        foreach( $users as $user ){
            $metas = get_user_meta( $user->ID, '_emp_pets', true );
            $userterms = array_unique (array_merge ($userterms, $metas));
        }
        ...
        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Pets Allowed */
        $search_types = nokri_get_cats2('emp_specialization_pets', $userterms , 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_pets']) && $_GET['emp_pets'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_pets"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_pets">
                    <form method="get" id="emp_job_pets"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_pets">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_pets'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_pets() {
    register_widget('nokri_search_employer_pets');
}

add_action('widgets_init', 'nokri_search_employer_pets');


/* ========================= */
/*  Employer Meal Plan widget  */
/* ========================= */

class nokri_search_employer_meal extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_meal', // Base ID
                esc_html__('Employer Meal Plan', 'nokri'), // Name
                array('description' => esc_html__('Show employer by meal', 'nokri'),) // Args
        );
    }
    ...
    public function widget($argss, $instance) {
        ...
        $userterms = array();
        foreach( $users as $user ){
            $metas = get_user_meta( $user->ID, '_emp_meal', true );
            $userterms = array_unique (array_merge ($userterms, $metas));
        }

        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Meal Plan */
        $search_types = nokri_get_cats2('emp_specialization_meal', $userterms , 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_meal']) && $_GET['emp_meal'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_meal"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_meal">
                    <form method="get" id="emp_job_meal"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_meal">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_meal'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_meal() {
    register_widget('nokri_search_employer_meal');
}

add_action('widgets_init', 'nokri_search_employer_meal');


/* ========================= */
/*  Employer Camping Available widget  */
/* ========================= */

class nokri_search_employer_camping extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_camping', // Base ID
                esc_html__('Employer Camping Available', 'nokri'), // Name
                array('description' => esc_html__('Show employer by camping', 'nokri'),) // Args
        );
    }
    ...
    public function widget($argss, $instance) {
        ...
        $userterms = array();
        foreach( $users as $user ){
            $metas = get_user_meta( $user->ID, '_emp_camping', true );
            $userterms = array_unique (array_merge ($userterms, $metas));
        }

        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Camping Available */
        $search_types = nokri_get_cats2('emp_specialization_camping', $userterms , 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_camping']) && $_GET['emp_camping'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                if ($showed) {
                    $showed = false;
                    $skill_html .= '<li class="show-more hide_now_' . esc_attr($cur_cls) . '"><small><a href="javascript:void(0);" class="show_records" data-attr-id="hide_skills" data-attr-hide="' . esc_attr($cur_cls) . '">' . __('', 'nokri') . '</a></small></li>';
                }

                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_camping"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_camping">
                    <form method="get" id="emp_job_camping"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_camping">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_camping'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_camping() {
    register_widget('nokri_search_employer_camping');
}

add_action('widgets_init', 'nokri_search_employer_camping');


/* ========================= */
/*  Employer Wifi Available widget  */
/* ========================= */

class nokri_search_employer_wifi extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_wifi', // Base ID
                esc_html__('Employer Wifi Available', 'nokri'), // Name
                array('description' => esc_html__('Show employer by wifi', 'nokri'),) // Args
        );
    }
    ...
    public function widget($argss, $instance) {
        ...
        $userterms = array();
        foreach( $users as $user ){
            $metas = get_user_meta( $user->ID, '_emp_wifi', true );
            $userterms = array_unique (array_merge ($userterms, $metas));
        }

        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Wifi Available */
        $search_types = nokri_get_cats2('emp_specialization_wifi', $userterms , 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_wifi']) && $_GET['emp_wifi'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_wifi"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_wifi">
                    <form method="get" id="emp_job_wifi"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_wifi">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_wifi'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_wifi() {
    register_widget('nokri_search_employer_wifi');
}

add_action('widgets_init', 'nokri_search_employer_wifi');


/* ========================= */
/*  Employer Cell Service widget  */
/* ========================= */

class nokri_search_employer_cell extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_cell', // Base ID
                esc_html__('Employer Cell Service', 'nokri'), // Name
                array('description' => esc_html__('Show employer by cell', 'nokri'),) // Args
        );
    }
    ...
    public function widget($args, $instance) {
        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Cell Service */
        $search_types = nokri_get_cats('emp_specialization_cell', 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_cell']) && $_GET['emp_cell'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_cell"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_cell">
                    <form method="get" id="emp_job_cell"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_cell">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_cell'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_cell() {
    register_widget('nokri_search_employer_cell');
}

add_action('widgets_init', 'nokri_search_employer_cell');


/* ========================= */
/*  Employer Staff Size widget  */
/* ========================= */

class nokri_search_employer_staff extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_staff', // Base ID
                esc_html__('Employer Staff Size', 'nokri'), // Name
                array('description' => esc_html__('Show employer by staff', 'nokri'),) // Args
        );
    }
    ...
    public function widget($args, $instance) {
        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Staff Size */
        $search_types = nokri_get_cats('emp_specialization_staff', 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_staff']) && $_GET['emp_staff'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_staff"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                <div class="panel-heading">
                    <a role="button" data-toggle="collapse" href="#emp_job_staff">
                        <?php
                        if (!empty($instance['title'])) {
                            echo nokri_returnEcho($args['before_title'] . apply_filters('widget_title', $instance['title']) . $args['after_title']);
                        }
                ...
                <div id="emp_job_staff">
                    <form method="get" id="emp_job_staff"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_staff">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_staff'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_staff() {
    register_widget('nokri_search_employer_staff');
}

add_action('widgets_init', 'nokri_search_employer_staff');



/* ========================= */
/*  Employer National Parks widget  */
/* ========================= */

class nokri_search_employer_natpark extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_natpark', // Base ID
                esc_html__('Employer National Parks', 'nokri'), // Name
                array('description' => esc_html__('Show employer by national Park', 'nokri'),) // Args
        );
    }
    ...
    public function widget($argss, $instance) {
        ...
        $userterms = array();
        foreach( $users as $user ){
            $metas = get_user_meta( $user->ID, '_emp_natpark', true );
            $userterms = array_unique (array_merge ($userterms, $metas));
        }

        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer National Parks */
        $search_types = nokri_get_cats2('emp_specialization_natpark', $userterms , 0);
        ...
        if (count((array) $search_types) > 0) {
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_natpark']) && $_GET['emp_natpark'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_natpark"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_natpark">
                    <form method="get" id="emp_job_natpark"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_natpark">
                                <option value=""><?php echo esc_html__("Select order", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_natpark'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_natpark() {
    register_widget('nokri_search_employer_natpark');
}

add_action('widgets_init', 'nokri_search_employer_natpark');



/* ========================= */
/*  Employer Additional regions widget  */
/* ========================= */

class nokri_search_employer_regions extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_regions', // Base ID
                esc_html__('Employer Additional regions', 'nokri'), // Name
                array('description' => esc_html__('Show employer by regions', 'nokri'),) // Args
        );
    }
    ...
    public function widget($args, $instance) {
        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Additional regions */
        $search_types = nokri_get_cats('emp_specialization_regions', 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_regions']) && $_GET['emp_regions'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_regions"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_regions">
                    <form method="get" id="emp_job_regions"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_regions">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_regions'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_regions() {
    register_widget('nokri_search_employer_regions');
}

add_action('widgets_init', 'nokri_search_employer_regions');


/* ========================= */
/*  Employer Additional States widget  */
/* ========================= */

class nokri_search_employer_states extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_states', // Base ID
                esc_html__('Employer Additional States', 'nokri'), // Name
                array('description' => esc_html__('Show employer by national Park', 'nokri'),) // Args
        );
    }
    ...
    public function widget($argss, $instance) {
        ...
        $userterms = array();
        foreach( $users as $user ){
            $metas = get_user_meta( $user->ID, '_emp_states', true );
            $userterms = array_unique (array_merge ($userterms, $metas));
        }

        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Additional States */
        $search_types = nokri_get_cats2('emp_specialization_states', $userterms , 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_states']) && $_GET['emp_states'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_states"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_states">
                    <form method="get" id="emp_job_states"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_states">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_states'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_states() {
    register_widget('nokri_search_employer_states');
}

add_action('widgets_init', 'nokri_search_employer_states');



/* ========================= */
/*  Employer Additional cities widget  */
/* ========================= */

class nokri_search_employer_cities extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_cities', // Base ID
                esc_html__('Employer Additional cities', 'nokri'), // Name
                array('description' => esc_html__('Show employer by cities', 'nokri'),) // Args
        );
    }
    ...
    public function widget($args, $instance) {
        /* Number Of Records */
        $no_of_records = !empty($instance['no_of_records']) ? $instance['no_of_records'] : esc_html__('Select Number', 'nokri');
        /*  Select Employer Additional cities */
        $search_types = nokri_get_cats('emp_specialization_cities', 0);
        if (count((array) $search_types) > 0) {
            $skill_html = '';
            $count = 1;
            $cls = '';
            ...
            foreach ($search_types as $search_type) {
                $selected = '';
                if (isset($_GET['emp_cities']) && $_GET['emp_cities'] == $search_type->term_id) {
                    $selected = 'selected = "selected"';
                }
                ...
                $skill_html .= '<option ' . esc_attr($selected) . ' value="' . esc_attr($search_type->term_id) . '" name="emp_cities"> <label>' . esc_html($search_type->name) . '</label>';
                $count++;
            }
            global $nokri;
            $in = '';
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_cities">
                    <form method="get" id="emp_job_cities"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="emp_cities">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                    <?php echo "" . ($skill_html); ?>
                        </select>
                        <?php echo nokri_search_params('emp_cities'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_cities() {
    register_widget('nokri_search_employer_cities');
}

add_action('widgets_init', 'nokri_search_employer_cities');
...
class nokri_job_keyword_new extends WP_Widget {
    ...
    public function widget($args, $instance) {
        ...
        ?>				
        <div id="homepage-search" class="panel panel-default">
            <div class="panel-heading">
                <label>
                    <?php echo nokri_returnEcho($instance['title'] ); ?>
                </label>
            </div>
            <div id="job_keyword search-widget">
                    ...
                    <form role="search" method="get"   action = "<?php echo get_the_permalink($nokri['sb_search_page']); ?>" class="custom-search-form">
                        <div class="form-group">
                            ...
                            <?php if (!$multi_searach) { ?> <button type="submit" class="btn"><i class="fa fa-search"></i></button> <?php } ?>
                        </div>
                        <div class="form-group form-action">
                        </div>
                        <?php
          ...
          <div class="list-body">
            ...
            <div class="list-meta">
              <ul class="">
                ...
                <li class="list-meta-with-icons single-d-views"> '. esc_html__('Employers','nokri').' : '. esc_html($no_of_employers).' </li>
              </ul>
            </div>
          </div>
        </li>';
...
/* ========================= */
/*  Employer International payment widget  */
/* ========================= */

class nokri_search_employer_payment extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'nokri_search_employer_payment', // Base ID
                esc_html__('Employer International Payment', 'nokri'), // Name
                array('description' => esc_html__('Do you accept internal payment?', 'nokri'),) // Args
        );
    }
    ...
    public function widget($argss, $instance) {
        ...
        $userterms = array();
        foreach( $users as $user ){
            $metas = get_user_meta( $user->ID, '_emp_currency');
            $userterms = array_unique (array_merge ($userterms, $metas));
        }
        ...
        if ( true ) {
            ...
            <div class="panel panel-default col-lg-4 col-md-4 col-sm-6 col-xs-6">
                ...
                <div id="emp_job_payment">
                    <form method="get" id="emp_job_payment"  action="<?php echo get_the_permalink($nokri['employer_search_page']); ?>">
                        <select onchange="this.form.submit()" class="js-example-basic-single form-control candidates_orders" data-allow-clear="true" data-placeholder="<?php echo esc_html__("Select Option", 'nokri'); ?>" name="_emp_currency">
                                <option value=""><?php echo esc_html__("Select Option", 'nokri'); ?>
                                
                                <?php 
                                foreach( $userterms as $terms ) {
                                ?>
                                    <option value="<?php echo $terms; ?>" <?php echo ( isset( $_GET['_emp_currency'] ) && $_GET['_emp_currency'] == $terms ? 'selected="selected"' : '' ); ?>><?php echo $terms; ?></option>
                                <?php 
                                }
                                ?>
                        </select>
                        <?php echo nokri_search_params('_emp_currency'); ?>
                        <?php echo nokri_form_lang_field_callback(true) ?>
                    </form>
                </div>
            </div>     
...
// register nokri Widget widget
function nokri_search_employer_payment() {
    register_widget('nokri_search_employer_payment');
}

add_action('widgets_init', 'nokri_search_employer_payment');
