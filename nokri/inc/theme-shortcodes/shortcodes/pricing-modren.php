		...
		foreach($rows as $row )
			{
				if( isset( $row['product'] ) )
				{
					...
					else if( get_post_meta( $row['product'], 'package_expiry_days', true ) != "" )
					{
						...
						$li.= '<li style="display:none;" ><i class="fa fa-check" aria-hidden="true"><span>'.get_post_meta( $row['product'], 'package_expiry_days', true ) . ' ' . __('Days','nokri').'</span></li>';
					}
					...
					if (get_post_meta( $row['product'], 'is_candidates_search', true ))
					{
						...
						{
							$li.= '<li><i class="fa fa-check" aria-hidden="true"><span>'.__('','nokri').' '.__('Applicant Search','nokri').'</span></li>';
						}
						else
						{
							if (get_post_meta( $row['product'], 'candidate_search_values', true ) )
							{
								$li.= '<li><i class="fa fa-check" aria-hidden="true"><span>'.__('Applicant Search','nokri').': '.get_post_meta( $row['product'], 'candidate_search_values', true ) . '</span></li>';
							}
						}
					}
...
//	$product_img = ( $product_imgeURL != "" ) ? ' \\s\\t\\y\\l\\e="background:  url('.$product_imgeURL.')  no-repeat; -webkit-background-size: contain; -moz-background-size: contain; -o-background-size: contain; background-size: contain; background-position: bottom center; background-attachment:scroll;"' : "";
    $product_img = ( $product_imgeURL != "" ) ? ''.$product_imgeURL.'' : "";
}
					...
					{
						$price_html =  '<div class="nth-pack-primium"><span class="n-ex">'.get_woocommerce_currency_symbol()."</span> ".$product->get_price() .'/month</div>';
					}
					
					$html	.= '<div class="product-container col-lg-3 col-md-3 col-sm-6 col-xs-12 ">
									<div class="nth-pck-table">
                                <div class="product-image"><img src="'.str_replace('\\',"",$product_img).'" /></div>
								<div class="nth-pckg-content">
									<div class="product-title"><span>'.get_the_title($row['product']).'</span></div>
                                    '.$price_html.'
									'.$pkg_details.'
								</div>
								<div class="nth-pck-details">
									<ul>
                                    <li><i class="fa fa-check" aria-hidden="true"><span>Employer Profile</span></li>
                                    <li><i class="fa fa-check" aria-hidden="true"><span>Unlimited Jobs</span></li>
