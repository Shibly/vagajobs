        ...
        <div class="col-md-6 col-xs-12 col-sm-6">
            <div class="form-group">
                ...
                <select class="js-example-basic-single emp_saved_resumes_form" name="c_order">
                    <option value="DESC" <?php if ( $c_order == 'DESC') { echo "selected"; } ; ?>><?php echo  esc_html__( 'Newest', 'nokri' ); ?></option>
                    <option value="ASC" <?php if ( $c_order == 'ASC') { echo "selected"; } ; ?>><?php echo  esc_html__( 'Oldest', 'nokri' ); ?></option>
                </select>
            </div>
        </div>
        </form>
...
foreach ( $cand_followers as $follower ) 
 ...
 /* Getting Followers Profile Photo */
	 ...
	 if(isset($image_dp_link[0]) && esc_url($image_dp_link[0])!= '')
	 {
		 $follower_img_html = '<div class="posted-job-title-img">
					<a href="'.esc_url(get_author_posts_url($follower_id)).'"><img src="'.esc_url($image_dp_link[0]).'" class="img-responsive" alt="'.esc_html__( 'Image', 'nokri' ).'"></a>
				</div>';
	 }
	 $headline_html = '';
 ...
 $li .= ' <div class="posted-job-list resume-on-jobs" id="company-box-'.esc_attr($follower_id).'">
		<ul class="list-inline">
			...
			<li class="posted-job-expiration"><a href="'.esc_url(get_author_posts_url($follower_id)).'" class="outlinebluebutton n-btn-flat btn-custom"> '.esc_html__( 'View Profile', 'nokri' ).' </a></li>
			<li class="posted-job-action"> 
				<a href="#" class="del_saved_resume"  data-id="'.$follower_id.'"><i class="fa fa-trash"></i></a>
			</li>
		</ul>
	</div>';
	$count++;
