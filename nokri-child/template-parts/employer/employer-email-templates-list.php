...
foreach( $resumes as $key => $val )
{
	...
	$resume_table .=  '<div class="posted-job-list" id="email_temp_del-'.$key.'">
						<ul class="list-inline">
							...
							<li class="posted-job-expiration">'.date_i18n(get_option('date_format'), strtotime($val['date'])).'</li>
							<li class="posted-job-action"> 
								<ul class="list-inline">
									<li class="tool-tip" title="'.esc_html__( 'View/Update', 'nokri' ).'"> <a href="'.esc_url($edit_link).'" class="label label-info"> <i class="fa fa-edit"></i></a></li>
									<li class="tool-tip del_email_template" data-tempId="'.esc_attr($key).'"  title="'.esc_html__( 'Delete', 'nokri' ).'"><a href="#" class="label label-danger"> <i class="fa fa-trash"></i></a></li>
								</ul>
							</li>
						</ul>
					</div>';
...
<div class="dashboard-posted-jobs">
    <div class="notification-box">
        ...
        <h4><?php echo esc_html__( 'No email template found', 'nokri' ); ?></h4>
    </div>
</div>
<?php } ?>
</div>
